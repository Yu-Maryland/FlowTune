...
# file, with "ftune" that tunes ABC synthesis flow automatically. (FreeTune (ftune) developed by Cunxi Yu at University of Utah)
# https://github.com/ycunxi/FreeTune-abc
# Usage:
...
########## FreeTune options for VTR ######################
#   Please first change the path of lib according to your vtr env setups.
#   Option:
#     -ftune_depth : Enable delay (targeting LUT Network Level) tuning 
#                : without -ftune_depth enable area (targeting number of 6-in LUTs) tuning
#   (This script is updated on July 13th, 2019, by Cunxi Yu (cunxi.yu@utah.edu) for Ftune experiments.
###################################################################################
...
my $ftune_target = 4; # 4=LUT SIZE Minimization ;  5=LUT Depth Minimization
	...
	and !$error_code )
{
    ...
    system("./single_design_run.sh ./temp/$odin_output_file_name 2 30");
...
my $abc_commands;
    if ($use_old_abc) {
        #Legacy ABC script
        $abc_commands="read $odin_output_file_name; time; st; if -K $lut_size; time; scleanup; time; scleanup; time; scleanup; time; scleanup; time; scleanup; time; scleanup; time; scleanup; time; scleanup; time; scleanup; time; write_hie $odin_output_file_name $abc_raw_output_file_name; print_stats";
    }
    ...
    if ($valgrind) {
            $valgrind = 0;
	    $q = &system_with_timeout( $abc_path, "abc.out", $timeout, $temp_dir, "-c", $abc_ftune_commands);
            $valgrind = 1;
    }
    else {
	    $q = &system_with_timeout( $abc_path, "abc.out", $timeout, $temp_dir, "-c", $abc_ftune_commands);
    }
    my $abc_ftune_cmd_file="$temp_dir\/$odin_output_file_name.script";
    open F, $abc_ftune_cmd_file or die "Can't open $abc_ftune_cmd_file";
        my $ftune_res=<F>;
    close F;
    print $ftune_res;
    # odin output blif goes to ABC
    
    $abc_commands="
	echo '';
	echo 'Load Netlist';
	echo '============';
	read $odin_output_file_name.ftune.blif;
	time;

	echo 'Load FTune Flow';
	echo '==========';
	read $odin_output_file_name.ftune.blif;
	echo '';
	echo 'Circuit Info';
...
";
    ...
    if ($valgrind) {
            $valgrind = 0;
	    $q = &system_with_timeout( $abc_path, "abc.out", $timeout, $temp_dir, "-c", $abc_commands);
            $valgrind = 1;
    }
    else {
        $q = &system_with_timeout( $abc_path, "abc.out", $timeout, $temp_dir, "-c", $abc_commands);
